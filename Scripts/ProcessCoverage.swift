...
/**
 Taken from https://github.com/remko/age-plugin-se/blob/main/Scripts/ProcessCoverage.swift
 under MIT License
 **/
...
try FileManager.default.createFile(
    atPath: outputPath,
    contents: JSONEncoder().encode(report)
)
...
extension CoverageSegment: Decodable {
    init(from decoder: Decoder) throws {
        ...
        self.line = try c.decode(Int.self)
        self.column = try c.decode(Int.self)
        self.count = try c.decode(Int.self)
        self.hasCount = try c.decode(Bool.self)
        self.isRegionEntry = try c.decode(Bool.self)
        self.isGapRegion = try c.decode(Bool.self)
    }
}
...
extension CoverageSegment: Encodable {
    func encode(to encoder: Encoder) throws {
        ...
        try c.encode(self.line)
        try c.encode(self.column)
        try c.encode(self.count)
        try c.encode(self.hasCount)
        try c.encode(self.isRegionEntry)
        try c.encode(self.isGapRegion)
    }
}
...
extension CoverageRegion: Decodable {
    init(from decoder: Decoder) throws {
        var c = try decoder.unkeyedContainer()
        self.lineStart = try c.decode(Int.self)
        self.columnStart = try c.decode(Int.self)
        self.lineEnd = try c.decode(Int.self)
        self.columnEnd = try c.decode(Int.self)
        self.executionCount = try c.decode(Int.self)
        self.fileID = try c.decode(Int.self)
        self.expandedFileID = try c.decode(Int.self)
        self.regionKind = try c.decode(Int.self)
    }
}
...
extension CoverageRegion: Encodable {
    func encode(to encoder: Encoder) throws {
        var c = encoder.unkeyedContainer()
        try c.encode(self.lineStart)
        try c.encode(self.columnStart)
        try c.encode(self.lineEnd)
        try c.encode(self.columnEnd)
        try c.encode(self.executionCount)
        try c.encode(self.fileID)
        try c.encode(self.expandedFileID)
        try c.encode(self.regionKind)
    }
}
...
extension String {
    ...
    var htmlEscaped: String {
        self.replacingOccurrences(of: "&", with: "&amp;").replacingOccurrences(
            of: "<", with: "&lt;"
