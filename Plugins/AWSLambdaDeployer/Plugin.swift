...
struct AWSLambdaDeployer: CommandPlugin {
  func performCommand(context: PackagePlugin.PluginContext, arguments: [String]) async throws {
    ...
    #else
      try self.compileSharedLibrary(
        projectDirectory: URL(fileURLWithPath: context.package.directory.string),
        buildConfiguration: configuration.buildConfiguration,
        swiftExecutable: swiftExecutablePath,
        verboseLogging: configuration.verboseLogging)
    #endif
    ...
    #if swift(>=6.0)
      try self.generateDeploymentDescriptor(
        projectDirectory: context.package.directoryURL,
        buildConfiguration: configuration.buildConfiguration,
        swiftExecutable: swiftExecutablePath,
    ...
    #if swift(>=6.0)
      try self.checkOrCreateSAMConfigFile(
        projetDirectory: context.package.directoryURL,
        buildConfiguration: configuration.buildConfiguration,
        region: awsRegion,
    ...
    #else
      try self.checkOrCreateSAMConfigFile(
        projetDirectory: URL(fileURLWithPath: context.package.directory.string),
        buildConfiguration: configuration.buildConfiguration,
        region: awsRegion,
        stackName: configuration.stackName,
        force: configuration.force,
    ...
    try self.validate(
      samExecutablePath: samExecutablePath,
      samDeploymentDescriptorFilePath: samDeploymentDescriptorFilePath,
      verboseLogging: configuration.verboseLogging)
    ...
    if !configuration.noList {
      let output = try self.listEndpoints(
        samExecutablePath: samExecutablePath,
        samDeploymentDescriptorFilePath: samDeploymentDescriptorFilePath,
        stackName: configuration.stackName,
        verboseLogging: configuration.verboseLogging)
  ...
  private func compileSharedLibrary(
  ...
  private func generateDeploymentDescriptor(
    projectDirectory: URL,
    buildConfiguration: PackageManager.BuildConfiguration,
    swiftExecutable: URL,
    shellExecutable: URL,
    ...
    do {
      ...
      if FileManager.default.fileExists(atPath: samDeploymentDescriptorFilePath) && !force {
        ...
        FileManager.default.createFile(
          atPath: samDeploymentDescriptorFilePath,
  ...
  private func checkOrCreateSAMConfigFile(
    projetDirectory: URL,
    buildConfiguration: PackageManager.BuildConfiguration,
    region: String,
    stackName: String,
  ...
  private func getDefaultAWSRegion(
    context: PluginContext,
    ...
    if let awsCLIPath = try? self.findExecutable(
      context: context,
      executableName: "aws",
...
private struct Configuration: CustomStringConvertible {
  ...
  public init(
    ...
    if let archiveDirectory = archiveDirectoryArgument.first {
      ...
      guard FileManager.default.fileExists(atPath: archiveDirectory, isDirectory: &isDirectory),
